...
BlockDecoder.prototype._decodeHeader = function () {
  ...
  if (!MAGIC_BYTES.equals(tap.buf.slice(tap.pos, tap.pos + MAGIC_BYTES.length))) {
...
BlockDecoder.prototype._writeChunk = function (chunk, encoding, cb) {
  ...
  while ((block = tryReadBlock(tap))) {
    if (!this._syncMarker.equals(block.sync)) {
      tap.pos = prevTapPos;
      // In case there are concatenated avro files, consume the header again.
      if (!this._decodeHeader()) {
        this.emit('error', new Error('invalid sync marker'));
        return;
      } else if (!(block = tryReadBlock(tap))) {  // read one more block
        break;
      } else if (!this._syncMarker.equals(block.sync)) {
        this.emit('error', new Error('invalid sync marker'));
        return;
      }
    }
